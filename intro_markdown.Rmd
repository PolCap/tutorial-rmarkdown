...
The example above is simple (showing the current date), but using the same syntax, you can display values produced by more complex R code. For example, calculating the min, median, and max of a result. For this example we will use the dataset `mtcars` from bas R. 
...
One powerful way to use R Markdown is to use code from other scripts. This can make your R Markdown script less cluttered, more simple, and easier to organize. It can also help if you want to display final figures at the beginning of the report. In this approach, the final R Markdown script simply combines pre-processed outputs into a document.
...
As you can see I could recover the plot created, but that's not all of it. I can reuse the function. 
